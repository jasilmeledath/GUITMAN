...
const validateAddProduct = async (req, res, next) => {
    const { product_name, price, stock, description, category, isActive, isTopModel } = req.body;
    const errors = {};
    ...
    try {
        ...
        // If validation fails, pass data and errors back to the form
        if (Object.keys(errors).length > 0) {
            ...
            req.session.formData = {
                ...
                isTopModel: isTopModel === "true" ? "true" : "false",
                images: req.files ? req.files.map((file) => file.filename) : [],
            };

            return res.render("backend/addProduct", {
                errors,
